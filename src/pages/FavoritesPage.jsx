...
const FavoritesPage = () => {
  const { favoriteGames, games } = useContext(MockAPIContext);
  const favoriteGameDetails = games.filter((game) => favoriteGames.includes(game.name));

  const { removeGameFromFavorites } = useContext(MockAPIContext);

  const handleRemoveFromFavorites = (gameName) => {
    removeGameFromFavorites(gameName);
  };
      ...
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={10}>
        {favoriteGameDetails.map((game, index) => (
          <Box key={index} borderWidth="1px" borderRadius="lg" overflow="hidden" p={6}>
            ...
            <Button colorScheme="red" onClick={() => handleRemoveFromFavorites(game.name)}>
